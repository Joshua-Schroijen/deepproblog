...
class Encoder(nn.Module):
    ...
    def forward(self, text, ent1, ent2):
        if globals.record_networks:
            with open("rel_extract_calibration_validation.txt", "a") as f:
                f.write(",".join([str(text), str(ent1), str(ent2)]) + "\n")
    ...
    def forward(self, text, ent):
        if globals.record_networks:
            with open("gender_net_calibration_validation.txt", "a") as g:
                gender_net_inputs = []
                text_list = term2list(text)
                for text_list_e in text_list:
                    gender_net_inputs.append(
                        Term(text_list_e.functor, text_list_e.args[0], "\"" + str(text_list_e.args[1]) + "\"")
                    )
                gender_net_inputs.append(ent)
                g.write(f"{str(gender_net_inputs)}\n")
